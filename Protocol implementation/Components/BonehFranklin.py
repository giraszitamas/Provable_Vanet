...
import math
def point_to_byte(P):
    x_byte = P.x.to_bytes(64,"big")
    y_byte = P.y.to_bytes(64,"big")
    return x_byte + y_byte

def byte_to_point(B, ec):
    x_byte = int.from_bytes(B[0:64],"big")
    y_byte = int.from_bytes(B[64:],"big")
...
class BonehFranklin_block(object):
    ...
    def H2(self, bp):
        ...
        BPhash = hashlib.sha256()
        BPhash.update(bp)
        BPhash = int(BPhash.hexdigest(), base=16).to_bytes(32,"big")
    ...
    def xor(self,s1, s2):
        ret = (s1[0] ^ s2[0]).to_bytes(1,"big")
        for i in range(1,len(s1)):
            ret+=(s1[i] ^ s2[i]).to_bytes(1,"big")
        return ret
